...
function initMarker() {
    ...
    var imagePath = path + '../resources/images/marker';

    // circle
    var blueDam = L.icon({
        iconUrl: imagePath + '/dam/status-dam-blue.png',
        iconSize: [17, 17]
    });
    var grayDam = L.icon({
        iconUrl: imagePath + '/dam/status-dam-gray.png',
        iconSize: [17, 17]
    });
    var greenDam = L.icon({
        iconUrl: imagePath + '/dam/status-dam-green.png',
        iconSize: [17, 17]
    });
    var minDam = L.icon({
        iconUrl: imagePath + '/dam/status-dam-min.png',
        iconSize: [17, 17]
    });
    var redDam = L.icon({
        iconUrl: imagePath + '/dam/status-dam-red.png',
        iconSize: [17, 17]
    });
    var yellowDam = L.icon({
        iconUrl: imagePath + '/dam/status-dam-yellow.png',
        iconSize: [17, 17]
    });
...
function initmap(cfg) {
    ...
    if (double_click_zoom == false) {
        map.doubleClickZoom.disable();
    }

    // load province boundary
    if (show_boundary == true) {
        geojson['province'] = new L.GeoJSON.AJAX(path + "../resources/json/boundary/thailand77.json", {
            style: {
                weight: 1,
                opacity: 0.7,
                color: 'black',
    ...
    if (layerLength <= 0) {
        baseMaps['Esri_WorldStreetMap'] = base[default_base_layers];
    } else {
    ...
    if (cfg.map.control.enabled_layer_control === true) {
