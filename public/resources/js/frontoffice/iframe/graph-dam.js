        ...
        if (rs.result == "OK"){
            ...
            for (var i = 0 ; i < gdLen ; i++){
                ...
                for(var j = 0 ; j < _dLen ; j++){
                    var d = _data[j];
                    var date = moment( JH.GetJsonValue(d, "date") );
                    date = parseInt( moment([graphDam.leapYear, date.month(), date.date()]).add(1, 'day').format('x'));
                ...
                if ( (curYear-1) == _year){
                    // ตั้งให้ ซีรี่ย์ เป็นสีน้ำเงิน
                    _seri["color"] = "#0000e6" ;
                }
                if ( (curYear-2) == _year){
                    // ตั้งให้ ซีรี่ย์ เป็นสีเขียว
                    _seri["color"] = "#004d00" ;
                }
